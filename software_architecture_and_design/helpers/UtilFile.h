...
inline void
makePath(std::string path) {
    if (path == "")
        return;
    size_t start = 0, end = 0;
    do {
        end = path.find("\\", start);
        _mkdir(end != std::string::npos ? path.substr(0, end + 1).c_str()
                                        : path.c_str());
        start = end + 1;
...
inline static char*
fileRead(const std::string& path, unsigned long long* fileSize = NULL,
         bool text = false) {
...
inline static char*
fileReadStr(const std::string& path) {
    return fileRead(path, 0, true);
}

inline bool
isImageExtension(const std::filesystem::path& filePath) {
...
inline static int
fileWrite(const std::string& name, const char* bulk, int len,
          bool append = false, bool exclusive = false) {
    ...
    if (append)
        ...
        int fd = _open(name.c_str(),
                       (exclusive ? _O_EXCL : 0) | _O_WRONLY | _O_CREAT |
                           _O_TRUNC | _O_BINARY,
                       _S_IREAD | _S_IWRITE);
...
inline static int
fileWriteStr(const std::string& name, std::string str, bool append = false,
             bool exclusive = false) {
    return fileWrite(name, str.c_str(), str.length(), append, exclusive);
}

inline static std::string
getFileStr(const std::string& path) {
...
inline static void
writeFileStr(const std::string& path, const char* buff, int size) {
...
inline static int
fileAppend(const std::string& name, std::string str) {
...
inline static int
fileWriteExclusive(const std::string& name, std::string str) {
